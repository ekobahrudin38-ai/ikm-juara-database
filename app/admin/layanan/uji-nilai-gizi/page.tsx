...
import { dummyIKMBinaan, dummyUjiNilaiGizi } from '@/lib/dummyData'

export default function UjiNilaiGiziPage() {
  const router = useRouter()
  const [data, setData] = useState(dummyUjiNilaiGizi.filter(d => !d.deleted_at).map(g => ({
    ...g,
    ikm: dummyIKMBinaan.find(ikm => ikm.id === g.ikm_binaan_id)
  })))
    ...
    <div className="flex">
      ...
      <main className="flex-1 p-8 bg-gray-50">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-800">Pendaftaran Uji Nilai Gizi</h1>
          ...
          <table className="w-full">
            <thead className="bg-[#2c3e50] text-white">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-sm">No. Laporan</th>
                <th className="px-4 py-3 text-left text-sm">Tanggal Hasil</th>
                <th className="px-4 py-3 text-left text-sm">Tahun</th>
                <th className="px-4 py-3 text-left text-sm">Laporan</th>
            ...
            <tbody>
              {data.map((item, index) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm font-mono">{item.nomor_laporan}</td>
                  <td className="px-4 py-3 text-sm">{item.tanggal_hasil}</td>
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-2xl w-full">
              <h2 className="text-xl font-bold mb-4">Tambah Data Uji Nilai Gizi</h2>
              <p className="text-gray-600 mb-4">Form tambah data Uji Nilai Gizi (Demo Mode)</p>
